...
t('벨트는 **끝에 닿은 개수**를 돌려준다 — 층수만큼 곱해서', () => {
  const B = { slots: [], head: 0 };
  /* 실제 belt.js 의 상태 꼴을 쓰기보다, 안 도는 조건만 확인한다 */
  assert.equal(S.runBelt(B, { speed: 0 }, 1), 0, '속도가 0인데 굴렀다');
  assert.equal(S.runBelt(B, { speed: 1 }, 0), 0, '시간이 0인데 굴렀다');
});
