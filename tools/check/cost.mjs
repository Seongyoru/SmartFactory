...
t('16:9 화면에서는 자동 계산이 **최소 높이로 붙는다** — 이 기능의 근거', () => {
  /* 이 값이 참이 아니게 되면(계산이 바뀌면) 잡아당기기의 이유가 달라진다 */
  assert.equal(D.dockHeight(1920, 1080), D.MIN_H, '16:9 에서 남는 몫이 생겼다');
  assert.equal(D.dockHeight(2560, 1440), D.MIN_H);
  /* 세로가 넉넉하면 계산이 제 몫을 한다 */
  assert.ok(D.dockHeight(1440, 1200) > D.MIN_H, '세로가 긴 화면에서도 최소값이다');
});
