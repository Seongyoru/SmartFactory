...
export function haltState(d = {}) {
  ...
  for (const f of beltFlows) {
     ...
     *  준다(`fullOf`).
     *
     *  **안 주면 이 벨트는 영영 안 선다.** 아래 줄이 「안 찼으면 건너뛴다」라
     *  `fullOf` 가 없을 때 `?? false` 로 떨어져 늘 건너뛰기 때문이다.
     *
     *  한때 이 자리에 **정반대의 말**이 적혀 있었다. 그 탓에 화면 쪽이 이 값을
     *  안 넘기는 것을 오래 못 봤다 — 헤드리스만 넘기고 있어서 같은 도면이
     *  화면과 반복 실행에서 다르게 돌았다. 반대말이 적혀 있으면 아무도 코드를
     *  안 본다.
     */
