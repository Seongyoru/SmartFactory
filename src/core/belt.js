...
/**
 * 칸 k 에 **몇 개** 실려 있나 (빈칸이면 0) — 화면이 몇 단을 세울지 여기서 본다.
 * ---------------------------------------------------------------------------
 *  덩어리가 늘 꽉 차지는 않는다. 로트 4개를 3개씩 내보내면 `3 + 1` 로 나뉘고,
 *  품종이 바뀌는 자리마다 그 자투리가 생긴다. 불량품을 내보내는 설비도 같다 —
 *  양품 사이에 불량품이 끼면 거기서 줄이 끊긴다.
 *
 *  그런데 화면은 **늘 층 수만큼 쌓아 그렸다.** 1개 실린 칸도 3단으로 보였고,
 *  그것이 적치대에 1개로 들어가니 「２개가 사라졌다」로 읽혔다. 사라진 것은
 *  없었다 — 애초에 1개였고 **그림만 거짓말을 했다.**
 */
export function beltLoad(st, k) {
  const n = st.counts?.length ?? 0;
  if (!n || st.born < 0) return 0;
  const c = wrap(st.born - k, n);
  /* `counts` 는 나중에 붙은 값이라, 옛 상태에는 0 이 들어 있을 수 있다.
     그때는 「있으면 한 개」로 본다 — 도착 셈(위)이 쓰는 것과 같은 규칙이다. */
  return st.counts[c] || (st.fill[c] === 1 ? 1 : 0);
}
