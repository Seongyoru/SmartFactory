...
/**
 * 로트를 덩어리로 쪼갤 때 **벨트 칸을 얼마나 헤프게 쓰나** — 1.0 이면 안 헤프다.
 * ---------------------------------------------------------------------------
 *  품종이 바뀌면 만들던 것을 끊고 실어 보낸다. 그때 남은 것이 한 덩어리를 못
 *  채우면 **1개짜리도 벨트 칸을 통째로** 쓴다. 로트 4 · 내보내기 3 이면
 *  `R×3 R×1` 이라 4개를 보내는 데 칸 두 개가 든다 — 이상적으로는 1.33 칸이다.
 *
 *      로트 3 · 3개씩   덩어리 1 / 이상 1.00 → 1.00   안 헤프다
 *      로트 4 · 3개씩   덩어리 2 / 이상 1.33 → 1.50   가장 헤프다
 *      로트 5 · 3개씩   덩어리 2 / 이상 1.67 → 1.20
 *      로트 6 · 3개씩   덩어리 2 / 이상 2.00 → 1.00
 *
 *  **벨트가 병목일 때만 손해가 된다.** 간격이 넉넉하면 칸을 헤프게 써도 안
 *  밀린다. 실측(짧은 벨트·전환 5초·두 품종, 30분): 로트 3 은 78개, 로트 4 는
 *  **63개** — 19% 손해다. 같은 도면에서 로트 6 은 96개가 나온다.
 *
 *  그래서 이 값은 「나쁘다」가 아니라 **「이만큼 헤프다」**를 말한다. 쓰는 쪽이
 *  벨트 사정을 보고 알릴지 정한다.
 */
export const slotWaste = (lot, per) => {
  const n = Math.max(1, Math.round(per) || 1);
  const l = Math.round(lot) || 0;
  if (l <= 0 || n <= 1) return 1;                 // 로트를 안 쓰거나 한 개씩 보내면 쪼갤 것이 없다
  return Math.ceil(l / n) / (l / n);
};
