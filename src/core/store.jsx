...
const initialState = {
  ...
  /**
   * 아래 띠(이번 실행 · 원가)를 펴 둘지.
   *  도면이 아니라 「지금 무엇을 보는 중인가」 라서 되돌리기·저장에는 안 넣는다.
   *  기본은 펴 둔 상태다 — 접혀 있으면 원가 기능이 있다는 것조차 모른다.
   */
  showRunDock: true,
  ...
  /**
   * 오더를 다 채워서 **스스로 멈춘 상태**인가.
   *  멈추면 화면의 모든 것이 얼어붙어 「페이지가 죽었나」 로 보인다. 왜 멈췄고
   *  어디를 누르면 다시 도는지를 화면이 짚어 줘야 한다(OrdersDock · 재생 버튼).
   *  도면이 아니라 지금 상태라서 저장·되돌리기에는 안 넣는다.
   */
  haltedByOrders: false,
