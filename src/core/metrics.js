 ...
 * ---------------------------------------------------------------------------
 *  이 도구는 재공(WIP)도 세고 처리량도 셌는데 **셋 중 하나가 비어 있었다.**
 *  세 값은 따로 노는 것이 아니라 하나로 묶여 있다 —
 *
 *      재공 = 처리량 × 리드타임          (리틀의 법칙)
 *
 *  납기를 다루는 도구에서 「이 주문이 언제 나오나」 의 근거가 바로 이것이다.
 *  오더 화면은 지금 **처리량만으로** 완료 시각을 미는데, 그러면 라인에 이미
 *  들어가 있는 것이 언제 나오는지는 못 말한다.
 *
 *  ── 개체를 따라다니지 않는다 ─────────────────────────────────────────────
 *  한 개 한 개에 들어온 시각을 찍어 두는 방법도 있다. 그러려면 벨트 위의 물건과
 *  카트에 실린 것과 재고 더미까지 전부 꼬리표를 달아야 하는데, 그렇게 얻는 것은
 *  **분포**(중앙값·최악)이고 여기서 필요한 것은 **평균**이다. 평균은 이미 있는
 *  두 값에서 나온다 — 새로 세지 않는다.
 *
 *  ── 정상 상태에서만 맞는다 ───────────────────────────────────────────────
 *  라인이 채워지는 동안에는 재공이 늘기만 하므로 리드타임이 부풀어 보인다.
 *  그래서 처리량과 **같은 조건**(WARMUP)에서만 답한다.
 *
 *  @param wip     지금 라인 안에 있는 개수 (재고 + 벨트 위 + 실린 것)
 *  @param perHour `throughput()` 의 값 — null 이면 아직 잴 때가 아니다
 *  @returns 초. 못 재면 null
 */
export function leadTimeSec(wip, perHour) {
  if (perHour == null || !(perHour > 0)) return null;
  const n = Math.max(0, wip ?? 0);
  return (n * 3600) / perHour;
}
