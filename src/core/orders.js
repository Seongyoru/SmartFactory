 ...
 */
export function ruleGap(rule, orders = [], kinds = [], feedsAll = false) {
  if (rule !== 'due' && rule !== 'behind') return null;

  /**
   * **하류가 두 품종을 함께 먹는데 규칙이 한 품종만 몰 수 있다.**
   * ---------------------------------------------------------------------------
   *  「납기 먼저」는 남은 납기가 급한 것을 고른다. 그런데 만든다고 납기가 늘지
   *  않는다 — 두 오더의 남은 납기가 **같은 속도로 줄어서 격차가 영영 안
   *  좁혀진다.** 그래서 규칙의 답이 **상수**가 된다. 실측: R 5분 · G 60분이면
   *  t=0 부터 3600초까지, 지금 무엇을 만들고 있든 늘 R 을 고른다.
   *
   *  그 자체는 EDD 가 원래 그런 것이라 틀리지 않았다. 문제는 **하류가 두 품종을
   *  함께 먹을 때**다 — 한쪽이 영영 안 만들어지니 조립기가 영영 굶는다. 실측:
   *  조립품 차례대로 66개 · 납기 먼저 **0개**, 600초 내내 100% 굶음.
   *
   *  라인이 굳은 뒤에 알리면 늦다. **고르는 자리에서** 미리 말한다.
   */
  if (feedsAll) {
    return rule === 'due'
      ? '아래 설비가 이 품종들을 **함께** 먹습니다 — 「납기 먼저」는 급한 하나만 계속 만들 수 있어, 나머지를 기다리는 쪽이 영영 굶을 수 있습니다. 「차례대로」가 안전합니다.'
      : '아래 설비가 이 품종들을 **함께** 먹습니다 — 오더 진척이 한쪽으로 쏠리면 나머지를 기다리는 쪽이 굶을 수 있습니다.';
  }
