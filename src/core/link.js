 ...
 *  갈래 — **어느 벨트로 무엇을 보내나**
 * --------------------------------------------------------------------------
 *  한 설비에서 벨트를 둘 이상 빼면 갈래가 생긴다. 아무것도 안 정하면 지금까지
 *  처럼 **먼저 집는 벨트가 가져간다** — 같은 것을 두 길로 나눠 보내는 도면이다.
 *
 *  벨트에 종류를 정해 두면 **그 종류만** 실어 간다. 제작품 1은 위쪽 벨트로,
 *  제작품 2는 아래쪽 벨트로 — 그것이 갈래(divert)다.
 *
 *  ── 못 싣는 종류는 **건너뛴다** ──────────────────────────────────────────
 *  앞머리가 이 벨트의 종류가 아니면 뒤를 본다(`simStore.takeBundles`). 안 그러면
 *  갈래를 나눠 놓고도 앞 종류가 안 빠져서 두 벨트가 함께 선다 — 갈래의 뜻이
 *  통째로 사라진다.
 *
 *  ── 합류는 **원래 됐다** ─────────────────────────────────────────────────
 *  벨트 여럿이 한 적치대·설비로 들어가는 것은 예전부터 된다. 흐름마다 제 종점에
 *  쌓으므로 따로 할 일이 없다. 새로 만든 것은 **갈래 쪽**이다.
 * ======================================================================== */

/**
 * **축적형 벨트인가** — 막혀도 안 서고 끝에 쌓인다.
 * ---------------------------------------------------------------------------
 *  기본은 아니다(비축적). 이미 그린 도면이 갑자기 버퍼를 얻으면 처리량이 저절로
 *  늘어서, 「내가 안 바꿨는데 값이 달라졌다」가 된다.
 */
export const isAccumulating = (link) => link?.accumulate === true;

/** 이 벨트가 실어 갈 종류들 — 빈 목록이면 **아무거나**(지금까지의 동작) */
export const beltKinds = (link) =>
  (Array.isArray(link?.kinds) ? link.kinds.filter((k) => typeof k === 'string' && k) : []);

/** 갈래가 잡힌 벨트인가 */
export const isDiverted = (link) => beltKinds(link).length > 0;
